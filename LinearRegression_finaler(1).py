...
#print (multi_X_test)
reg = LinearRegression()
reg.fit(multi_X_train, data_y_train)
print ("Regression coefficients: %.9f, %.9f" % (reg.coef_[:,0],reg.coef_[:,1]))
split = cross_val_score(reg, multi_X, data_y, cv=5)
print ("Cross val score: %.9f, %.9f, %.9f, %.9f, %.9f" % (split[0],split[1],split[2],split[3],split[4]))
lowestError = split_average = ((split[0]+split[1]+split[2]+split[3]+split[4])/5)
print ("Error for 5 KFold: %.8f" % split_average)
print("Lowest Error set to %.9f" % lowestError)
...
split = cross_val_score(reg, multi_X, data_y, cv=5)
print ("Cross val score: %.9f, %.9f, %.9f, %.9f, %.9f" % (split[0],split[1],split[2],split[3],split[4]))
split_average= ((split[0]+split[1]+split[2]+split[3]+split[4])/5)
print ("Error for 5 KFold: %.8f" % split_average)
if (lowestError > split_average and split_average>0):
    lowestError = split_average
    print("Lowest Error Changed to %.9f" % lowestError)
print()
